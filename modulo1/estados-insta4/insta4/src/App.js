...
const App = () => {
  ...
  const [inputFotoUsuatrio, setInputFotoUsuatrio] = useState('');
  const [inputFotoPost, setInputFotoPost] = useState('');

  const adicionaPostagem = () => {
    const novaPostagem = {
      nomeUsuario: inputUsuario,
      fotoUsuario: inputFotoUsuatrio,
      fotoPost: inputFotoPost

    };

    const novasPostagens = [...arrayPost.posts, novaPostagem];
  ...
  const onChangeInputFotoUsuatrio = (event) => {
  ...
  const listaPost = arrayPost.posts.map((posted) =>{
    return (
      <Post 
  ...
  <MainContainer>
    <div>
      ...
      <input
        ...
        />
        <input
          value={inputFotoUsuatrio}
          onChange={onChangeInputFotoUsuatrio}
